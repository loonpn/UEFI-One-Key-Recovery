...
//g_data.bExitToShell = TRUE;
	...
	*/
	MyLog(LOG_DEBUG, L"[AUTOBR][AUTOBR][AUTOBR][AUTOBR][AUTOBR][AUTOBR]\n");
	MyLog(LOG_DEBUG, L"[AUTOBR]Auto Backup Restore Func Start.\n");
	int iRet = IDB_SHUTDOWN;
	g_data.bIsAutoFunc = FALSE;
	BYTE iAutoFuncFlag = 0;
	if (!GetAutoFuncFlagFromConfigFile(&iAutoFuncFlag))
	{
		GetAutoFuncFlagFromBIOS(&iAutoFuncFlag);
		MyLog(LOG_DEBUG, L"[AUTOBR]After GetAutoFuncFlagFromBIOS iAutoFuncFlag= %d.\n", iAutoFuncFlag);
	}
	else
	{
		MyLog(LOG_DEBUG, L"[AUTOBR]After GetAutoFuncFlagFromConfigFile iAutoFuncFlag= %d.\n", iAutoFuncFlag);
		if (0 != iAutoFuncFlag)
		{
			MyLog(LOG_DEBUG, L"[AUTOBR]Call ClearAutoFuncFlagInConfigFile.\n");
			ClearAutoFuncFlagInConfigFile();
		}
	}
	
	MyLog(LOG_DEBUG, L"[AUTOBR]Match Func iAutoFuncFlag= %d.\n", iAutoFuncFlag);
	//getch();
	switch (iAutoFuncFlag)
	{
	case 0:
	{
		//0x00h:进入主界面，走Base中的流程
		MyLog(LOG_DEBUG, L"[AUTOBR]Enter Main Face.\n");
		iRet = Face();
		break;
	}
	case 1:
	{
		//0x01h:自动备份功能
		MyLog(LOG_DEBUG, L"[AUTOBR]Enter AutoBackup Face.\n");
	...
	{
		//0x02h:自动恢复功能
		MyLog(LOG_DEBUG, L"[AUTOBR]Enter AutoRestore Face.\n");
		g_data.bIsAutoFunc = TRUE;
		iRet = AutoRestoreFace();
		break;
	}
	case 3:
	{
		//0x03h:强制恢复功能
		MyLog(LOG_DEBUG, L"[AUTOBR]Enter ForceRestore Face.\n");
		g_data.bIsAutoFunc = TRUE;
		iRet = ForceRestoreFace();
		break;
	}
	default:
		break;
...
cleanup:
	...
	{
		if (iRet == IDB_SHUTDOWN)
		{
			gRT->ResetSystem(EfiResetShutdown, EFI_SUCCESS, 0, NULL);
		}
		else //if (iRet == IDB_REBOOT)
		{
