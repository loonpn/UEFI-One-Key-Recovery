...
DWORD		m_SectorsPerCluster;   //每簇的扇区数

BYTE		m_PartitionType;       //分区类型
BYTE		m_PartitionIndex;      //分区数
BYTE		m_DiskNumber;          //硬盘数
BYTE		m_bLBAMode;            
...
#if OKR_DEBUG
		MyLog(LOG_DEBUG, L"CalcPartitionSize totalUsedClusters 0x%llx, validsector 0x%llx, Partition->FreeSectors 0x%llx.\n", totalUsedClusters, validsector, Partition->FreeSectors);
#endif
...
BOOL  CheckPartitionType(BYTE* Buffer, BOOL bGetFreeClr, WIN32_PARTITIONINFO *Partition)
{
	...
	// "-FVE-FS-" bitlocker
	if (strnicmp((CHAR8*)pVol->signature, "-FVE-FS-", 8) == 0)
	{		
		MyLog(LOG_DEBUG, L"Detect Bitlocker partition......\r\n");
		//MyLogBinary(Buffer, 4 * 512);
#if OKR_DEBUG
		MyLog(LOG_DEBUG, L" sector_size %d\r\n GUID: %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x \r\n sectors_per_cluster: %d\r\n reserved_clusters: %d\r\n fat_count: %d\r\n root_entries: %d\r\n nb_sectors_16b: %d\r\n media_descriptor: %d\r\n sectors_per_fat: %d\r\n sectors_per_track: %d\r\n nb_of_heads: %d\r\n hidden_sectors: %ld\r\n nb_sectors_32b: %ld\r\n nb_sectors_64b: %ld\r\n mft_start_cluster: %ld\r\n mft_mirror %ld\r\n\r\n",
			pVol->sector_size, 
			pVol->guid[0], pVol->guid[1], pVol->guid[2], pVol->guid[3], pVol->guid[4], pVol->guid[5], pVol->guid[6], pVol->guid[7], pVol->guid[8], 
			pVol->guid[9], pVol->guid[10], pVol->guid[11], pVol->guid[12], pVol->guid[13], pVol->guid[14], pVol->guid[15],
			pVol->sectors_per_cluster,
			pVol->reserved_clusters,
			pVol->fat_count, 
			pVol->root_entries,
			pVol->nb_sectors_16b,
			pVol->media_descriptor,
			pVol->sectors_per_fat,
			pVol->sectors_per_track,
			pVol->nb_of_heads,
			pVol->hidden_sectors,  ///start sector?
			pVol->nb_sectors_32b,
			pVol->nb_sectors_64b,  //total sector?
			pVol->mft_start_cluster,
			pVol->mft_mirror);
#endif
		m_PartitionType = SHD_BITLOCKER;
	}

	// "MSWIN4.1" for BitLocker-To-Go encrypted volumes
	if (strnicmp((CHAR8*)pVol->signature, "MSWIN4.1", 8) == 0)
	{
		//MyLogBinary(Buffer, 4*512);
		m_PartitionType = SHD_BITLOCKERTOGO;
		MyLog(LOG_DEBUG, L"Detect BitLocker-To-Go partition......\r\n");
		//MyLogBinary(Buffer, 4 * 512);
	}

	if(m_PartitionType != NTFS && m_PartitionType != FAT32 && m_PartitionType != FAT16 && m_PartitionType != SHD_BITLOCKER && m_PartitionType != SHD_BITLOCKERTOGO)
	{
	...
	//判读扇区字节数是否为512字节的整数倍
	//MyLog(LOG_DEBUG, L"p16->SecInByte %x \r\n", p16->SecInByte);
	if (p16->SecInByte == 0 || (p16->SecInByte & 0x1ff) || p16->SecPerClr == 0)
	{
		m_PartitionType = PARTITION_UNUSED;
		//MyLog(LOG_DEBUG, L"Check if the bytes of sector is 512 * n, m_PartitionType PARTITION_UNUSED\r\n");
	}
	...
	{
		m_PartitionType = PARTITION_UNUSED;
		//MyLog(LOG_DEBUG, L"Check SecInByte is 2^n  m_PartitionType PARTITION_UNUSED\r\n");

	}

	//判断secperclr是否为2的n次方
	//MyLog(LOG_DEBUG, L"p16->SecPerClr %x \r\n", p16->SecPerClr);
	if ((p16->SecPerClr & (p16->SecPerClr - 1)) != 0)
	{
		m_PartitionType = PARTITION_UNUSED;
		//MyLog(LOG_DEBUG, L"Check secperclr is 2^n  m_PartitionType PARTITION_UNUSED\r\n");
	}
	...
	else if (m_PartitionType == SHD_BITLOCKER)
	{
		Partition->PartitionType = SHD_BITLOCKER;
#if OKR_DEBUG
		MyLog(LOG_DEBUG, L"Partition->TotalSectors %ld\r\n", Partition->TotalSectors);
#endif
	}
	else if (m_PartitionType == SHD_BITLOCKERTOGO)
	{
		Partition->PartitionType = SHD_BITLOCKERTOGO;
#if OKR_DEBUG
		MyLog(LOG_DEBUG, L"Partition->TotalSectors %ld\r\n", Partition->TotalSectors);
#endif
	}
...
{
	//MyLog(LOG_DEBUG, L"ParseMasterPartition...\r\n");
	ULONGLONG bpbsector;
    BOOL bRet = TRUE;
	...
	if ( bRet )
	{
		...
		if (!CheckPartitionType(Buffer, TRUE, &g_Partitions[PartitionIndex]))
		{ 
			//boot sector invalid
			//g_Partitions[PartitionIndex].PartitionId = UNKNOWN_PARTITION;;
...
DWORD  GetPartitionInfo()
{
	...
	BYTE Window_Recovery_TypeGuidTemp[16] = { 0xa4,0xbb,0x94,0xde,0xd1,0x06,0x40,0x4d,0xa1,0x6a,0xbf,0xd5,0x01,0x79,0xd6,0xac };
	for(i=0;i<g_dwTotalPartitions;i++) {
		GetVolumeName(i);

		if ((StrnCmp(g_Partitions[i].Label, L"LENOVO_PART", 11) == 0 &&
			MyMemCmp(Window_Recovery_TypeGuidTemp, g_Partitions[i].TypeGuid, 16) == 0) ||
			g_Partitions[i].PartitionId == LENOVO_PART)
		{
			g_data.oempartition = i;
			//MyLog(LOG_DEBUG, L"g_data.oempartition %d.\r\n", g_data.oempartition);
		}
	}
...
{
#if OKR_DEBUG
	MyLog(LOG_DEBUG, L"ParseGPTInfo...\r\n");
#endif
...
//	MyLog(LOG_DEBUG,L"Buffer %p, Entry %p, PartitionLba 0x%lx, mylba 0x%ld", Buffer, entry, header->PartitionEntryLBA, header->MyLBA);
	...
	for (i = 0; i < num; i++)
	{
		...
		if (entry->PartitionTypeGUID.Data1 == 0 &&
			entry->PartitionTypeGUID.Data2 == 0 &&
			entry->PartitionTypeGUID.Data3 == 0)
		{
			//MyLog(LOG_DEBUG, L"invalid entry: Type GUID is 0.\r\n");
			bValidEntry = FALSE;
		}

		if (entry->StartingLBA == 0 || entry->EndingLBA == 0)
		{
			//MyLog(LOG_DEBUG, L"invalid entry: StartingLBA or entry->EndingLBA is 0.\r\n");
			bValidEntry = FALSE;
		}

		//MyLog(LOG_DEBUG,L"ParseGPTInfo %d, bValid %d\r\n", i, bValidEntry);
		if (bValidEntry)
		{
#if OKR_DEBUG
			MyLog(LOG_DEBUG,L"ParseGPTInfo, index %d, start %ld, end %ld total %ld\r\n", g_dwTotalPartitions, entry->StartingLBA, entry->EndingLBA, entry->EndingLBA - entry->StartingLBA + 1);
#endif
...
VOID GetVolumeName(WORD partindex)
{
	//MyLog(LOG_DEBUG, L"GetVolumeName  partindex %d  .\n", partindex);
	...
	if (type == FAT32 || type == FAT32_E){
		...
		if (DiskRead(disknum, beginsec, 1, buffer) == FALSE)
		{
			goto cleanup;
		}
		...
		if (DiskRead(disknum, beginsec + RootStartSec, 1, buffer) == FALSE)
		{
			goto cleanup;
		}
...
//#endif
		...
		buffersize = (g_Partitions[partindex].TotalSectors + (BLOCK_SIZE-1))/BLOCK_SIZE;
