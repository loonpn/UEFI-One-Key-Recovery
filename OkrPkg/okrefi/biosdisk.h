...
#define SHD_BITLOCKER					0x0D5
#define SHD_BITLOCKERTOGO				0x0D6
...
typedef struct WIN32_PARTITIONINFO
{
	...
	BYTE	PartitionType;		//40	NTFS/Fat32/FAT12/FAT16/bitlocker/SHD_BITLOCKER/SHD_BITLOCKERTOGO/PARTITION_LINUX/PARTITION_UNUSED
...
typedef struct _NTFS_BOOT_SECTOR	//NTFS格式的参数块
{
	BYTE	Jump[3];				//0
	BYTE	OemID[8];				//3
	WORD	BytesPerSector;			//11  0xb
	BYTE	SecPerClr;				//13  0xd
	WORD	BootSectors;			//14  0xe
	BYTE	Mbz1;					//16  0x10
	WORD	Mbz2;					//17  0x11
	WORD	Reserved1;				//19  0x13
	BYTE	MediaType;				//21  0x15
	WORD	Mbz3;					//22  0x16
	WORD	SecPerTrk;				//24  0x18
	WORD	Sides;					//26  0x1A
	DWORD	PartitionOffset;		//28  0x1C
	DWORD	Reserved2[2];			//32  0x20
	ULONGLONG TotalSectors;			//40  0x28
	ULONGLONG MftStartLcn;			//48  0x30
	ULONGLONG Mft2StartLcn;			//56  0x38
	DWORD	ClustersPerFileRecord;	//64  0x40
	DWORD	ClustersPerIndexBlock;	//68  0x44
	DWORD	SerialNumber;			//72  0x48
	DWORD	SerialNumber1;			//76  0x4C
	BYTE	Code[0x1AE];			//80  0x50
	WORD	BootSignature;
...
{
	/* 512 bytes long */
	BYTE  jump[3];             //                                                -- offset 0
	BYTE  signature[8];        // = "-FVE-FS-" (without 0 at the string's end)   -- offset 3
							   // = "NTFS    " (idem) for NTFS volumes (ORLY?)
							   // = "MSWIN4.1" for BitLocker-To-Go encrypted volumes

	WORD sector_size;         // = 0x0200 = 512 bytes                           -- offset 0xb    ; 扇区字节数
	BYTE  sectors_per_cluster; //                                               -- offset 0xd    ; 每簇扇区数
	WORD reserved_clusters;   //                                                -- offset 0xe
	BYTE  fat_count;           //                                               -- offset 0x10
	WORD root_entries;        //                                                -- offset 0x11    ; root directory entries
	WORD nb_sectors_16b;      //                                                -- offset 0x13    ; total number of sectors(16-bit)
	BYTE  media_descriptor;    //                                               -- offset 0x15    ; sectors per file allocation table(FAT)
	WORD sectors_per_fat;     //                                                -- offset 0x16    ; sectors per track 0x3f(63)
	WORD sectors_per_track;   //                                                -- offset 0x18    ; 每磁道扇区数
	WORD nb_of_heads;         //                                                -- offset 0x1a    ; number of heads
	DWORD hidden_sectors;      //                                               -- offset 0x1c    ; number of hidden sectors. Contains the volume start sensor number
	DWORD nb_sectors_32b;      //                                               -- offset 0x20    ; total number of sectors(32-bit)

	union {                       //                                                -- offset 0x24
		struct { // Classic BitLocker
			BYTE  unknown2[4];         // NTFS = 0x00800080 (little endian)
			ULONGLONG nb_sectors_64b;      //                                       -- offset 0x28  ; total number of sectors(64-bit)
			ULONGLONG mft_start_cluster;   //                                       -- offset 0x30  
			union {                       // Metadata LCN or MFT Mirror             -- offset 0x38
				ULONGLONG metadata_lcn;    //  depending on whether we're talking about a Vista volume
				ULONGLONG mft_mirror;      //  or an NTFS one
			};
			BYTE  unknown3[96];        //                                        -- offset 0x40

			BYTE  guid[16];                //                                        -- offset 0xa0
			ULONGLONG information_off[3];  // NOT for Vista                          -- offset 0xb0
			ULONGLONG eow_information_off[2]; // NOT for Vista NOR 7                 -- offset 0xc8

			BYTE  unknown4[294];       //                                        -- offset 0xd8
		};
		struct { // BitLocker-To-Go
			BYTE  unknown5[35];

			BYTE  fs_name[11];         //                                        -- offset 0x47
			BYTE  fs_signature[8];     //                                        -- offset 0x52

			BYTE  unknown6[334];       //                                        -- offset 0x5a

			BYTE  bltg_guid[16];           //                                    -- offset 0x1a8
			ULONGLONG bltg_header[3];      //                                    -- offset 0x1b8

			BYTE  Unknown7[46];        //                                        -- offset 0x1d0
		};
	};

	WORD boot_partition_identifier; // = 0xaa55                                  -- offset 0x1fe
} volume_header_t, *pvolume_header_t; // Size = 512
...
typedef struct 
{
	EFI_TABLE_HEADER  Header;					//0         GPT头
	EFI_LBA           MyLBA;					//0x18		存储GPT头的扇区的地址
	EFI_LBA           AlternateLBA;				//0x20		备份表头的地址
	EFI_LBA           FirstUsableLBA;			//0x28		可用于分区的第一个扇区的地址
	EFI_LBA           LastUsableLBA;			//0x30		可用于分区的最后一个扇区的地址
	EFI_GUID          DiskGUID;					//0x38		硬盘GUID
	EFI_LBA           PartitionEntryLBA;		//0x48		分区表地址
	DWORD             NumberOfPartitionEntries;	//0x50		分区表想的数目
	DWORD             SizeOfPartitionEntry;		//0x54		每个分区表项的字节数
	DWORD             PartitionEntryArrayCRC32;	//0x58		分区表的CRC32码
...
///
typedef struct {
	EFI_GUID  PartitionTypeGUID;		//0					分区表类型GUID，0表示此项为空
	EFI_GUID  UniquePartitionGUID;		//16				分区标志GUID
	EFI_LBA   StartingLBA;				//32				分区的首扇区
	EFI_LBA   EndingLBA;				//40				分区的尾扇区
	ULONGLONG Attributes;				//48				分区属性
	WCHAR     PartitionName[36];		//56 + 72 = 128		分区名字符串，必须以0x0000结尾
} EFI_PARTITION_ENTRY;
#endif
