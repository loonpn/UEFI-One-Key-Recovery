	...
	if ((dev->d_ops->open)(dev, NVolReadOnly(vol) ? O_RDONLY: O_RDWR)) {
		if (!NVolReadOnly(vol) && (errno == EROFS)) {
			if ((dev->d_ops->open)(dev, O_RDONLY)) {
				ntfs_log_perror("Error opening read-only '%p'",
						dev->d_efidev->DiskHandle);
				goto error_exit;
			} else {
				ntfs_log_info("Can only open '%p' as read-only\n",
						dev->d_efidev->DiskHandle);
				NVolSetReadOnly(vol);
			}
		} else {
			ntfs_log_perror("Error opening '%p'", dev->d_efidev->DiskHandle);
			goto error_exit;
		}
	}
 ...
 */
ntfs_volume *ntfs_mount(EFI_DISK_PARAMETER *efidev __attribute__((unused)),
		ntfs_mount_flags flags __attribute__((unused)))
{
	...
	dev = ntfs_device_alloc(efidev, 0, &ntfs_device_default_io_ops, NULL);
	if (!dev)
		return NULL;
...
static int empty_journal(ntfs_volume *vol)
{
  ...
  int fix_mount(EFI_DISK_PARAMETER *device, unsigned long flags)
{
	...
	dev = ntfs_device_alloc(device, 0, &ntfs_device_default_io_ops,
			NULL);
	if (!dev) {
		ntfs_log_info(FAILED);
	...
	flags = 0;//(opt.no_action ? NTFS_MNT_RDONLY : 0);
	vol = ntfs_volume_startup(dev, flags);
