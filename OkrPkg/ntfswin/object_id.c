 ...
 *
 *	According to RFC 4122, GUID_NTFSs should be printed with the most
 ...
 *	and, for indexing, GUID_NTFSs are compared as if they were a sequence
			...
			const OBJECT_ID_ATTR *object_id)
{
	...
	memcpy(&indx.key.object_id,object_id,sizeof(GUID_NTFS));

	indx.data.file_id = file_id;
	memcpy(&indx.data.birth_volume_id,
			&object_id->birth_volume_id,sizeof(GUID_NTFS));
	memcpy(&indx.data.birth_object_id,
			&object_id->birth_object_id,sizeof(GUID_NTFS));
	memcpy(&indx.data.domain_id,
			&object_id->domain_id,sizeof(GUID_NTFS));
			...
			OBJECT_ID_ATTR *full_objectid)
{
	...
	if (xo) {
		memcpy(&key.object_id,objectid_attr,sizeof(GUID_NTFS));
				...
				OBJECT_ID_ATTR *old_attr)
{
	...
	if (ret) {
		...
		size = ntfs_attr_pread(na, 0, sizeof(GUID_NTFS), old_attr);
		if (size >= (s64)sizeof(GUID_NTFS)) {
			memcpy(&key.object_id,
				&old_attr->object_id,sizeof(GUID_NTFS));
			if (!ntfs_index_lookup(&key,
					sizeof(OBJECT_ID_INDEX_KEY), xo)) {
				entry = (struct OBJECT_ID_INDEX*)xo->entry;
 ...
 *	non-duplication of the GUID_NTFS should have been checked before.
 *
 *	Returns 0 if success
 *		-1 if failure, explained by errno
			...
			const OBJECT_ID_ATTR *value, size_t size)
{
	...
	if (na) {
		...
		if (oldsize < 0)
			...
			/* resize attribute */
			res = ntfs_attr_truncate(na, (s64)sizeof(GUID_NTFS));
				/* write the object_id in attribute */
			if (!res && value) {
				written = (int)ntfs_attr_pwrite(na,
					(s64)0, (s64)sizeof(GUID_NTFS),
					&value->object_id);
				if (written != (s64)sizeof(GUID_NTFS)) {
 ...
 *
 *	If present, the object_id from the attribute and the GUID_NTFSs
...
int ntfs_get_ntfs_object_id(ntfs_inode *ni, char *value, size_t size)
{
	...
	if (ni) {
		...
		if (objectid_attr) {
				/* restrict to only GUID_NTFS present in attr */
			if (attr_size == sizeof(GUID_NTFS)) {
				memcpy(&full_objectid.object_id,
						objectid_attr,sizeof(GUID_NTFS));
				full_size = sizeof(GUID_NTFS);
			...
			const char *value, size_t size, int flags)
{
	...
	res = 0;
	if (ni && value && (size >= sizeof(GUID_NTFS))) {
		xo = open_object_id_index(ni->vol);
		if (xo) {
			/* make sure the GUID_NTFS was not used somewhere */
			memcpy(&key.object_id, value, sizeof(GUID_NTFS));
			if (ntfs_index_lookup(&key,
					sizeof(OBJECT_ID_INDEX_KEY), xo)) {
				ntfs_index_ctx_reinit(xo);
				...
				if (!res) {
					...
					/* GUID_NTFS is present elsewhere */
				res = -1;
				errno = EEXIST;
			}
			xoni = xo->ni;
