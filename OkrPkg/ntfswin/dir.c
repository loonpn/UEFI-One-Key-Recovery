...
static int ntfs_filldir(ntfs_inode *dir_ni, s64 *pos, u8 ivcn_bits,
		const INDEX_TYPE index_type, /*index_union*/void *iu, INDEX_ENTRY *ie,
		void *dirent, ntfs_filldir_t filldir)
{
	...
	int res;
	MFT_REF mref;
	INDEX_ROOT *ir=(INDEX_ROOT*)iu;
	INDEX_ALLOCATION *ia=(INDEX_ALLOCATION*)iu;

	ntfs_log_trace("Entering.ivcn_bits=%u,index_type=%d,iu=%p,ie=%p,dir_ni->vol=%p\n",ivcn_bits,index_type,iu,ie,dir_ni->vol);
...
// 		ntfs_log_trace("ia=%p\n",
// 			ia);
// 		ntfs_log_trace("ia=%p,ia->index_block_vcn=%lld\n",
// 			ia,ia->index_block_vcn);
// 		ntfs_log_trace("ia=%p,ia->index_block_vcn=%lld,dir_ni->vol->mft_record_size=%lld\n",
// 			ia,ia->index_block_vcn,dir_ni->vol->mft_record_size);

		*pos = (u8*)ie - (u8*)ia + (sle64_to_cpu(
				ia->index_block_vcn) << ivcn_bits) +
		...
		void *dirent, ntfs_filldir_t filldir)
{
	...
	if (!dir_ni || !pos || !filldir) {
		errno = EINVAL;
		ntfs_log_perror("invalid para.dir_ni=%p,pos=%p,filldir=%p\n",dir_ni,pos,filldir);
		return -1;
	}

	if (!(dir_ni->mrec->flags & MFT_RECORD_IS_DIRECTORY)) {
		errno = ENOTDIR;
		ntfs_log_perror("not dir.dir_ni->mrec->flags=%u\n",dir_ni->mrec->flags);
		return -1;
	}
	...
	for (;; ie = (INDEX_ENTRY*)((u8*)ie + le16_to_cpu(ie->length))) {
		...
		rc = ntfs_filldir(dir_ni, pos, index_vcn_size_bits,
				INDEX_TYPE_ROOT,(void*) ir, ie, dirent, filldir);
		if (rc) {
			ntfs_attr_put_search_ctx(ctx);
			ctx = NULL;
			goto err_out;
...
find_next_index_buffer:
	...
	for (;; ie = (INDEX_ENTRY*)((u8*)ie + le16_to_cpu(ie->length))) {
		 ...
		 */
		rc = ntfs_filldir(dir_ni, pos, index_vcn_size_bits,
				INDEX_TYPE_ALLOCATION, (void*)ia, ie, dirent, filldir);
		if (rc)
			goto err_out;
	}
		...
		const ntfschar *target, int target_len)
{
	...
	if (NVolHideDotFiles(dir_ni->vol)
		 ...
		 */
	if (dir_ni->flags & FILE_ATTR_HIDDEN)
	{
		ni->flags |= FILE_ATTR_HIDDEN;
	}
