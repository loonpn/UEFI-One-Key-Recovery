...
static struct ntfs_logging ntfs_log = {
	...
	NTFS_LOG_FLAG_ONLYNAME|NTFS_LOG_FLAG_FILENAME|NTFS_LOG_FLAG_LINE|NTFS_LOG_FLAG_FUNCTION,
#ifdef DEBUG
 ...
 */
extern CHAR16 * Ascii2Unicode (OUT CHAR16         *UnicodeStr, IN  CHAR8          *AsciiStr);
extern VOID MyLog(WORD loglevel, IN  CONST CHAR16  *Format,  ...);
VOID MyLogStringA(char *lpString)
{
	CHAR16 buffer1[1500];
	Ascii2Unicode(buffer1,lpString);
	MyLog(5,buffer1);
}
...
//	ret = ntfs_log.handler(function, file, line, level, data, format, args);//ysy 0217
	va_end(args);
	
//	MyLogStringA(buffer);//ysy 0217
