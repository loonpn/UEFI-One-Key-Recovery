...
#define CHINA_TIMEZONE_OFFSET	(-8 * SECONDS_IN_HOUR)	//define china timezone offset from greenwich time 
...
static __inline__ ntfs_time ntfs_current_time(void)
{
	...
	now.tv_sec = microseconds.tv_sec + CHINA_TIMEZONE_OFFSET;
	now.tv_nsec = microseconds.tv_usec * 1000;
#else
	now.tv_sec = time((time_t*)NULL) + CHINA_TIMEZONE_OFFSET;
	now.tv_nsec = 0;
#endif
