...
extern BOOL ntfs_guid_is_zero(const GUID_NTFS *guid);
extern char *ntfs_guid_to_mbs(const GUID_NTFS *guid, char *guid_str);
...
extern char *ntfs_sid_to_mbs(const SID *sid, char *sid_str,
		size_t sid_str_size);
extern void ntfs_generate_guid(GUID_NTFS *guid);
...
int ntfs_set_ownmod(struct SECURITY_CONTEXT *scx,
		ntfs_inode *ni, uid_t uid, gid_t gid,const mode_t mode);
#if POSIXACLS
