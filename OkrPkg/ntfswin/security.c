...
static const GUID_NTFS __zero_guid = { const_cpu_to_le32(0), const_cpu_to_le16(0),
		const_cpu_to_le16(0), { 0, 0, 0, 0, 0, 0, 0, 0 } };
static const GUID_NTFS *const zero_guid = &__zero_guid;

/**
 * ntfs_guid_is_zero - check if a GUID_NTFS is zero
 * @guid:	[IN] guid to check
 *
 * Return TRUE if @guid is a valid pointer to a GUID_NTFS and it is the zero GUID_NTFS
 * and FALSE otherwise.
 */
BOOL ntfs_guid_is_zero(const GUID_NTFS *guid)
{
	return (memcmp(guid, zero_guid, sizeof(*zero_guid)));
}

/**
 * ntfs_guid_to_mbs - convert a GUID_NTFS to a multi byte string
 * @guid:	[IN]  guid to convert
 * @guid_str:	[OUT] string in which to return the GUID_NTFS (optional)
 *
 * Convert the GUID_NTFS pointed to by @guid to a multi byte string of the form
 * "XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX".  Therefore, @guid_str (if not NULL)
 * needs to be able to store at least 37 bytes.
 *
 * If @guid_str is not NULL it will contain the converted GUID_NTFS on return.  If
 ...
 */
char *ntfs_guid_to_mbs(const GUID_NTFS *guid, char *guid_str)
{
	char *_guid_str;
	int res;
 ...
 * @guid:	[OUT]   pointer to a GUID_NTFS struct to hold the generated guid.
 *
 * perhaps not a very good random number generator though...
 */
void ntfs_generate_guid(GUID_NTFS *guid)
{
	unsigned int i;
	u8 *p = (u8 *)guid;
...
int ntfs_get_owner_mode(struct SECURITY_CONTEXT *scx,
		ntfs_inode * ni, struct ntfs_stat *stbuf)
{
	const SECURITY_DESCRIPTOR_RELATIVE *phead;
	char *securattr;
	const SID *usid;	/* owner of file/directory */
			...
			BOOL allowdef)
{
	...
	if (usermap_path[0] == '/') {
		fd = open(usermap_path,O_RDONLY,0);
		if (fd > 0) {
