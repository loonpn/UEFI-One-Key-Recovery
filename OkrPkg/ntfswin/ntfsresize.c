...
#define  perr_printf(fmt, ...) MyLogA(fmt, ##__VA_ARGS__)
 
#define  err_printf(fmt, ...) MyLogA(fmt, ##__VA_ARGS__)
 
#define  err_exit(fmt, ...) MyLogA(fmt, ##__VA_ARGS__)
 
#define  perr_exit(fmt, ...) MyLogA(fmt, ##__VA_ARGS__)

 
 

void exit_exit(int a)
{
	return;
}
/**
...
static void usage(int ret)
{
	...
	MyLogA("\nUsage: %s [OPTIONS] DEVICE\n"
		...
		"\n", EXEC_NAME);
	MyLogA("%s%s", ntfs_bugs, ntfs_home);
	MyLogA("Ntfsresize FAQ: http://linux-ntfs.sourceforge.net/info/ntfsresize.html\n");
	exit(ret);
}
...
static void proceed_question(void)
{
		...
		MyLogA(" CHANGES  will   made to your "
				"NTFS volume.\n");
...
static void version(void)
{
	...
	MyLogA("Copyright (c) 2002-2006  Szabolcs Szakacsits\n");
	MyLogA("Copyright (c) 2002-2005  Anton Altaparmakov\n");
	MyLogA("Copyright (c) 2002-2003  Richard Russon\n");
	MyLogA("Copyright (c) 2007       Yura Pakhuchiy\n");
	MyLogA("Copyright (c) 2011-2016  Jean-Pierre Andre\n");
...
extern s64 get_volume_size(void);
static s64 get_new_volume_size(void)
{
	 

	return get_volume_size();
}
...
static void print_advise(ntfs_volume *vol, s64 supp_lcn)
{
	...
	/* WARNING: don't modify the text, external tools grep for it */
	if (!opt.infombonly)
		MyLogA("You might resize at %lld bytes ", (long long)new_b);
	if ((new_mb * NTFS_MBYTE) < old_b) {
		if (!opt.infombonly)
			MyLogA("or %lld MB ", (long long)new_mb);
		else
			MyLogA("Minsize (in MB): %lld\n", (long long)new_mb);
	}

	if (!opt.infombonly) {
		MyLogA("(freeing ");
		if (freed_mb && (old_mb - new_mb))
			MyLogA("%lld MB", (long long)(old_mb - new_mb));
		else
		        MyLogA("%lld bytes", (long long)freed_b);
		MyLogA(").\n");

		MyLogA("Please make a test run using both the -n and -s "
...
static void collect_relocation_info(ntfs_resize_t *resize, runlist *rl)
{
	...
	MyLogA("Relocation needed for inode %8lld attr 0x%x LCN 0x%08llx "
			"length %6lld\n", (long long)inode,
		...
		for (j = 0; j < lcn_length; j++) {
			...
			if (k >= (u64)vol->nr_clusters) {
				...
				if (++fsck->show_outsider <= 10 || opt.verbose)
					MyLogA("Outside of the volume reference"
			...
			if (ntfs_bit_get_and_set(lcn_bitmap->bm, k, 1)) {
				if (++fsck->multi_ref <= 10 || opt.verbose)
					MyLogA("Cluster %lld is referenced "
					       "multiple times!\n",
					       (long long)k);
				continue;
			}
...
static void compare_bitmaps(ntfs_volume *vol, struct bitmap *a)
{
		...
		for (i = 0; i < count; i++, pos++) {
			...
			for (cl = pos * 8; cl < (pos + 1) * 8; cl++) {
				...
				if (!mismatch && !bit && !backup_boot &&
					...
					backup_boot = 1;
					MyLogA("Found backup boot sector in "
				...
				MyLogA("Cluster accounting failed at %lld "
						"(0x%llx): %s cluster in "
						"$Bitmap\n", (long long)cl,
	...
	if (mismatch) {
		MyLogA("Filesystem check failed! Totally %d cluster "
 ...
 */
 extern void okrupdatepercent(int a);
 extern void okrupdatepercent_last(void);
static void progress_update(struct progress_bar *p, u64 current)
{
	...
	if (current != p->stop) {
		if ((current - p->start) % p->resolution)
			return;
		MyLogA("%d percent completed\r", percent);
		okrupdatepercent(percent);
	} else
		MyLogA("100.00 percent completed\n");
	fflush(stdout);
}
...
static int build_allocation_bitmap(ntfs_volume *vol, ntfsck_t *fsck)
{
	...
	/* WARNING: don't modify the text, external tools grep for it */
        if (!opt.infombonly)
		MyLogA("Checking filesystem consistency ...\n");
...
static void set_resize_constraints(ntfs_resize_t *resize)
{
	...
	ntfs_inode *ni;
        ...
        if (!opt.infombonly)
		MyLogA("Collecting resizing constraints ...\n");
okrupdatepercent_last();
...
static void delayed_updates(ntfs_resize_t *resize)
{
	...
	if (nr_extents > 2) {
		MyLogA("WARNING: The MFT is now severely fragmented"
			" (%d extents)\n", nr_extents);
	}
}
...
static void copy_clusters(ntfs_resize_t *resize, s64 dest, s64 src, s64 len)
{
	...
	char *buff = malloc(NTFS_MAX_CLUSTER_SIZE);
	...
	for (i = 0; i < len; i++) {
		...
		if (read_all(vol->dev, buff, vol->cluster_size) == -1) {
			...
			if (errno == EIO)
				if(buff)
					free(buff);
				MyLogA("%s", bad_sectors_warning_msg);
			exit_exit(1);
		}
...
static void advise_on_resize(ntfs_resize_t *resize)
{
	...
	if (opt.verbose) {
		MyLogA("Estimating smallest shrunken size supported ...\n");
		MyLogA("File feature         Last used at      By inode\n");
...
static int check_bad_sectors(ntfs_volume *vol)
{
	...
	if (badclusters) {
		MyLogA("%sThis software has detected that the disk has at least"
		       " %lld bad sector%s.\n",
		...
		if (!opt.badsectors) {
			...
			MyLogA("WARNING: Bad sectors can cause reliability "
...
static ntfs_volume *mount_volume(void)
{
	...
	if (!opt.infombonly) {
		MyLogA("Device name        : %s\n", opt.volume);
		MyLogA("NTFS volume version: %d.%d\n",
...
{
	MyLogA("Schedule chkdsk for NTFS consistency check at Windows boot "
			"time ...\n");
	...
	if (vol->dev->d_ops->sync(vol->dev) == -1)
		perr_exit("Failed to sync device");
	MyLogA("Resetting $LogFile ... (this might take a while)\n");
...
static BOOL can_expand(expand_t *expand, ntfs_volume *vol)
{
	...
	MyLogA("Sectors allocated to volume :  old %lld current %lld difference %lld\n",
			(long long)old_sector_count,
			(long long)(old_sector_count + expand->sector_increment),
			(long long)expand->sector_increment);
	MyLogA("Clusters allocated to volume : old %lld current %lld difference %lld\n",
	...
	if (ok && opt.info) {
		...
		MyLogA("You must expand the partition to at least %lld bytes,\n",
			(long long)(minimum_size + vol->sector_size));
		MyLogA("and you may add a multiple of %ld bytes to this size.\n",
			(long)vol->cluster_size);
		MyLogA("The minimum NTFS volume size is %lld bytes\n",
...
static int really_expand(expand_t *expand)
{
	...
	if (expand->bitmap
		...
		MyLogA("\n*** WARNING ***\n\n");
		MyLogA("Expanding a volume is an experimental new feature\n");
		if (!opt.ro_flag)
			MyLogA("A first check with option -n is recommended\n");
		MyLogA("\nShould something go wrong during the actual"
			 " resizing (power outage, etc.),\n");
		MyLogA("just restart the procedure, but DO NOT TRY to repair"
			" with chkdsk or similar,\n");
		MyLogA("until the resizing is over,"
		...
		/* rewriting the backup bootsector, no return ticket now ! */
				...
				MyLogA("Cannot check further in no-action mode\n");
			}
			free(expand->bootsector);
			free(expand->mrec);
		}
...
static int expand_to_beginning(void)
{
	...
	ret = -1;
	dev = ntfs_device_alloc((EFI_DISK_PARAMETER *)opt.volume, 0, &ntfs_device_default_io_ops,
			NULL);
	if (dev) {
...
int resize_main(int argc, char **argv)
{
	...
	device_size = 0x7000000000 ;//ntfs_device_size_get(vol->dev, vol->sector_size); kang
	...
	if (!opt.info && !opt.infombonly && (new_size == vol->nr_clusters ||
		...
		MyLogA("Nothing to do: NTFS volume size is already OK.\n");
		return 0;
	}
	...
	if (opt.ro_flag) {
		MyLogA("The read-only test run ended successfully.\n");
		return 0;
	}

	/* WARNING: don't modify the texts, external tools grep for them */
	MyLogA("Syncing device ...\n");
	if (vol->dev->d_ops->sync(vol->dev) == -1)
		perr_exit("fsync");

	MyLogA("Successfully resized NTFS on device  .\n");
	if (resize.shrink)
		MyLogA("%s", resize_important_msg);
