...
extern  int fix_mount(EFI_DISK_PARAMETER *device, unsigned long flags);
/**
 * utils_mount_volume - Mount an NTFS volume
 */
ntfs_volume * utils_mount_volume(EFI_DISK_PARAMETER *device, unsigned long flags)
{
	ntfs_volume *vol;
...
// 		return NULL;
	...
	if (!vol) {
		...
		{
			// reset logfile and remount
			
			fix_mount(device,flags);
...
int utils_cluster_in_use(ntfs_volume *vol, long long lcn)
{
	...
	static long long bmplcn = -(((long long)sizeof(buffer) << 3));
	int byte, bit;
	ntfs_attr *attr;
...
int utils_mftrec_in_use(ntfs_volume *vol, MFT_REF mref)
{
	...
	static s64 bmpmref = -((s64)sizeof(buffer) << 3) - 1; /* Which bit of $BITMAP is in the buffer */
	int byte, bit;
...
// 			ntfs_log_debug("\e[%dm", col);
		if (off == s)
			ntfs_log_dump("%6.6x ", start);
		else
			ntfs_log_dump("%6.6x ", off);

		for (i = 0; i < 16; i++) {
			if ((i == 8) && (!(flags & DM_NO_DIVIDER)))
				ntfs_log_dump(" -");
			if (((off+i) >= start) && ((off+i) < (start+length)))
				ntfs_log_dump(" %02X", mem[off+i]);
			else
				ntfs_log_dump("   ");
		}
